# Phase 4.1 — Cloud project schema + RLS + publish/join

You are implementing **Phase 4.1** of the Verbalis Translation IDE revamp.

**Goal**: Add the cloud project schema (projects, project_members with roles, ydoc_state, ydoc_updates) with member-scoped RLS, plus publishProject/openCloudProject flows.
...
2. Read `docs/revamp/ROADMAP.md` §4 Phase 4.1 — that section is the **complete and only** scope definition (files to create/modify, key design points, tests, DoD). Also read the §3 architecture decisions it cites. Do not exceed that scope.
...
## Deliver
- Branch `claude/revamp-phase-4-1` created from latest `main`.
- Draft PR titled `Revamp 4.1: Cloud project schema + RLS + publish/join`.
...
- If scope overflows the session: ship the core slice green, append the remainder as Phase 4.1.1 to STATUS and ROADMAP §4, and say so in the PR body.
