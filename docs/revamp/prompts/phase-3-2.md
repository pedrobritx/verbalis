# Phase 3.2 — Microsoft + Apple providers + account settings

You are implementing **Phase 3.2** of the Verbalis Translation IDE revamp.

**Goal**: Add Azure and Apple OAuth buttons gated by VITE_AUTH_PROVIDERS, and the account settings section (display name, linked identities, sign out).
...
2. Read `docs/revamp/ROADMAP.md` §4 Phase 3.2 — that section is the **complete and only** scope definition (files to create/modify, key design points, tests, DoD). Also read the §3 architecture decisions it cites. Do not exceed that scope.
...
## Deliver
- Branch `claude/revamp-phase-3-2` created from latest `main`.
- Draft PR titled `Revamp 3.2: Microsoft + Apple providers + account settings`.
...
- If scope overflows the session: ship the core slice green, append the remainder as Phase 3.2.1 to STATUS and ROADMAP §4, and say so in the PR body.
